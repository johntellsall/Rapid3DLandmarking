...
def landmark_face(filename):
    mesh = _read_mesh(filename)
    mesh = _clean_mesh(mesh)
    landmarks = _identify_3D_landmarks(mesh)
    _save_landmarks(landmarks, filename)


def _identify_3D_landmarks(mesh):
...
def _save_landmarks(landmarks, filename):
    landmark_file = filename[:-3]+'txt'
...
def _read_mesh(mesh_file):
    ...
    print("Only ply and obj files are supported")
